...
class Hamiltonian:
    ...
    def bdg_self_consistency(self, U, T, max_iter=100,
                             tol=1e-6, atol=1e-6, rtol=1e-6,
                             with_corr = False):
        flag = False
        ...
        for iteration in range(max_iter):
            ...
            for i in range(num_sites):
                ...
                sl = slice(4*i, 4*(i+1))
                self.matrix[sl, sl][:2, 2:] = -1j * gap_new[i] * s2
                self.matrix[sl, sl][2:, :2] = (-1j * gap_new[i] * s2).conj().T

            if cp.max(cp.abs(gap_new)) < cp.max(cp.abs(gap)) and cp.max(cp.abs(gap_new)) < tol:
                print("Gap collapsed to zero.")
                gap = gap_new
                flag = True
                break

            if cp.max(cp.abs(gap_new - gap)) < atol + rtol * cp.max(cp.abs(gap_new)):
    ...
    def current_self_consistency(self, U, T, max_iter=100,
                             tol=1e-6, with_corr=False):
        flag = False
        num_sites = self.lattice.num_sites
        gap  = self.gap.copy()
        gap0 = self.gap.copy()

        X,Y = self.lattice.X, self.lattice.Y
        ...
        for iteration in range(max_iter):
            ...
            y = cp.arange(Y, dtype=cp.int64)
            left_idx  = X * y
            right_idx = (X - 1) + X * y
            gap_new[left_idx]  = gap0[left_idx]
            gap_new[right_idx] = gap0[right_idx]
            ...
            # if (mag_diff < tol) and (phase_err < tol):
            if cp.linalg.norm(cp.abs(gap_new - gap)) < tol:
                print(f"Converged after {iteration+1} iterations.")
                gap = gap_new
                flag = True
                break
