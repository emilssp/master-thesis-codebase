...
theta_arr = np.linspace(0, np.pi/2 + 1e-12, 10)

def main():
    idx = int(sys.argv[1])
    idx_temp = idx % len(temps)
    idx_theta = idx // len(temps)
    ...
    hx = h0 * np.cos(theta_arr[idx_theta])
    hy = h0 * np.sin(theta_arr[idx_theta])
    ...
    os.makedirs("data/DFP_hdir", exist_ok=True)
    np.savez(
        f"data/DFP_hdir/temp_{idx:04d}.npz",
        idx=idx,
        temp=temp,
    ...
    if idx == 1:
        ...
        param_file = f"data/DFP_hdir/params_{idx:04d}.txt"
