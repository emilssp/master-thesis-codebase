...
def main():
    ...
    partial_sc(H, temperature=temp,
            ...
            atol=1e-6, rtol=1e-3, maxiter=100, 
            verbose=False, mixing=0.6)

    F0 = H.F0
    F, Fuu, Fdd = H.get_correlations()

    os.makedirs("data/DFP_V6", exist_ok=True)
    np.savez(
        f"data/DFP_V6/temp_{idx:04d}.npz",
        idx=idx,
        temp=temp,
        converged=H.converged,
        free_energy=H.free_energy(temp),
        F0=F0[int(widthS/2)],
    ...
    if idx == 1:
        ...
        param_file = f"data/DFP_V6/params_{idx:04d}.txt"
